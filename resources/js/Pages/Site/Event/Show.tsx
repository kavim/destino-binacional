...
export default function Show({ event }: { event: SiteEventShow }) {
    ...
    const mapEmbed =
        !event.is_online && event.google_maps_src
            ? normalizeGoogleMapsEmbedSrc(event.google_maps_src)
            : undefined;
                                ...
                                {event.is_online ? (
                                    ...
                                    <div className="flex items-center gap-2 text-stone-200">
                                        ...
                                        <span className="text-center md:text-left">
                                            {locationLine || 'Local a confirmar'}
                                        </span>
                                    </div>
                                )}
                            </div>
                                    ...
                                    {event.is_online ? (
                                        ...
                                        <div className="space-y-4">
                                            <p className="text-sm font-medium leading-relaxed text-gray-600 dark:text-muted-foreground">
                                                {locationLine || 'Endereço não informado.'}
                                            </p>
                                            {mapsOpenUrl ? (
                                                <a
                                                    href={mapsOpenUrl}
                                                    target="_blank"
                                                    rel="noopener noreferrer"
                                                    className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-primary/30 bg-primary/10 px-4 py-3 text-sm font-semibold text-primary transition hover:bg-primary/15 dark:border-primary/40"
                                                >
